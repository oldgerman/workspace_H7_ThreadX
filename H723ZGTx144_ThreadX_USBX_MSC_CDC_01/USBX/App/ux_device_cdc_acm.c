...
#endif
*/
__attribute__((section(".axisram2_bss"), aligned(4))) uint8_t UserRxBufferFS[APP_RX_DATA_SIZE];
__attribute__((section(".axisram2_bss"), aligned(4))) uint8_t UserTxBufferFS[APP_TX_DATA_SIZE];

uint32_t UserTxBufPtrIn;
uint32_t UserTxBufPtrOut;
#if EN_USB_USRAT_BRIDGE
UART_HandleTypeDef *uart_handler;
#endif
extern TX_EVENT_FLAGS_GROUP EventFlagCDC;
  ...
  while (1)
  {
    ...
    if ((device->ux_slave_device_state == UX_DEVICE_CONFIGURED) && (cdc_acm != UX_NULL))
    {
      ...
      if (actual_length != 0)
      {
        ...
        if (tx_event_flags_get(&EventFlagCDC, TX_NEW_TRANSMITTED_DATA, TX_OR_CLEAR,
...
VOID usbx_cdc_acm_write_thread_entry(ULONG thread_input)
{
  ...
  while (1)
  {
    ...
    if (tx_event_flags_get(&EventFlagCDC, RX_NEW_RECEIVED_DATA, TX_OR_CLEAR,
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (tx_event_flags_set(&EventFlagCDC, TX_NEW_TRANSMITTED_DATA, TX_OR) != TX_SUCCESS)
  {
    Error_Handler();
  }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (tx_event_flags_set(&EventFlagCDC, RX_NEW_RECEIVED_DATA, TX_OR) != TX_SUCCESS)
  {
    Error_Handler();
  }
...
static VOID USBD_CDC_VCP_Config(UX_SLAVE_CLASS_CDC_ACM_LINE_CODING_PARAMETER
                                *CDC_VCP_LineCoding)
{
	UX_PARAMETER_NOT_USED(CDC_VCP_LineCoding);
#if EN_USB_USRAT_BRIDGE
